...
# (3. F+W)
# 4. Thymus segmentation, volumetrics and fat-water ratios.
# 5. Validation against manually segmented reference mask.
...
if ( scalar(@ARGV) < 3 ) {
  print "Usage: ThymusDixonQuantificationCL2.pl <SubjectFile> <Input Data Path> <Command line parameters>\n";
  print "Thymus quantification using original fat and water Dixon images as input to segmentation.\n";
  exit;
}
...
while(<IN>) {
	...
	$autosegmentationMask = "$outputPath" . "/" . "${subjectID}_AutoMask$formatExtension";
	$thymusQuantificationCommand = "$thymusquantificationProgram  --subjectid ${subjectID} --seed $coordR,$coordA,$coordI --outputdir $outputPath --outmask $autosegmentationMask $thymusquantificationParameters $croppedWSFilename $croppedFSFilename";
